...
if (parseInt(querystring(0)) < 1) {
// document.getElementById('remover').style.display='none';
document.getElementById('userid').value = localStorage.lgi;
...
if (querystring(0) > '-1') { 
spjson(dic_path+'snapsect_api.php','act=get&id='+querystring(0)+'&lang='+localStorage.language,'GET', function(qq) {
// alert(JSON.stringify(qq.snapsect));
mostra(qq);
}, function(e) { }); 
// // mostra(lst_snapsect[parseInt(querystring(0))]);
}
else {
...
//document.getElementById('lstpg').value = tela;
...
function delimage() {
spopen(dic_path+'snapsect_api.php','act=delimage&id='+document.getElementById('id').value,'POST', function(res) {
document.getElementById('photodesc').value='';
document.getElementById('grfoto').src = '';
});
} // da funcao.

function mostra(ress) {
var res = ress.snapsect;
var users = ress.users;
var dados = res;
// document.getElementById('id').value=dados.id;
if (dados.userid < 1) { dados.userid = localStorage.lgi; }
document.getElementById('userid').value=dados.userid;
document.getElementById('status').value=dados.status;
if (dados.status == 1) { document.getElementById('chkstatus').checked = true; }
...
var marcado = (res.ground.indexOf('|'+users[r].uid+'|') > -1)? ' checked':'';
gt+='<li><input type="checkbox" id="chk'+users[r].uid+'" onclick="';
gt+="javascript:if (this.checked) { document.getElementById('ground').value +='|'+"+users[r].uid+"+'|'; } else { document.getElementById('ground').value = document.getElementById('ground').value.replaceAll('|'+"+users[r].uid+"+'|',''); }";
gt+='" '+marcado+'><label for="chk'+users[r].uid+'"> '+users[r].nome+'</label></li>';
} // do for...
gt+='</ul>';
document.getElementById('dvground').innerHTML = gt;
arrumaclasse();
fala(dic[229]+' '+dic[dados.classe]+', '+dados.commonname);
} // da funcao...
...
// if (document.getElementById('commonname').value.length > 0) {
// if (document.getElementById('scientificname').value.length > 0) {
var temp='act=add&lang='+localStorage.language;
temp+='&id='+document.getElementById('id').value;
temp+='&userid='+document.getElementById('userid').value;
