...
function getvoiceconfirm(t,okfunc,nofunc,cfunc,ofunc) {
getvoice(t, function(resultado) {
var c = resultado.toUpperCase();
if (dic[478].toUpperCase().indexOf(c) > -1) {
if (!okfunc) { } else { okfunc(); }
} // sim...
else {
if (dic[504].toUpperCase().indexOf(c) > -1) {
if (!nofunc) { } else { nofunc(); }
} // nao...
else {
if (dic[477].toUpperCase().indexOf(c) > -1) {
if (!cfunc) { } else { cfunc(); }
} else {
if (!ofunc) { } else {  ofunc(resultado); }
} // nao e o cancelar...
} // nao e nao...
} // nao e sim
});
...
function getvoicetext(t, okfunc, nofunc, ofunc) {
getvoiceconfirm(t, function() {
getvoice(dic[546]+':', function(rr) {
fala(dic[542]+': '+rr, function() {
if (!okfunc) { } else { okfunc(rr); }
});
});
}, function() {
if (!nofunc) { } else { nofunc(); }
...
if (!ofunc) { 
getvoicetext(t, okfunc, nofunc, ofunc);
} else { ofunc(res); }
});
} // da funcao...

function getvoicelist(t,lista, okfunc, nofunc, ofunc) {
getvoiceconfirm(t, function() {
getvoice(dic[541]+': '+lista, function(rr) {
var p = lista.indexOf(rr);
if (p > -1) { 
fala(dic[457]+': '+lista[p], function() {
if (okfunc) { } else { okfunc(p); }
});
} else {
getvoicelist(t,lista, okfunc, nofunc, ofunc);
} // nao optou por nenhuma opcao...
});
}, function() {
if (!nofunc) { } else { nofunc(); }
}, function() {
...
function editopc(o,r, nextfunc) {
if (r < 0) { r=0; }
if (r < o.length) {
var t = o.options[r].text+': seleciona esta opção?';
getvoiceconfirm(t, function() {
o.value = o.options[r].value;
if (o.onchange) { 
o.onchange(); 
}
fala(dic[453]+' '+o.options[r].text, function() {
if (!nextfunc) { } else { nextfunc(); }
});
}, function() {
editopc(o,r+1, nextfunc);
}, function() {
fala(dic[442], startcommand);
}, function(res) {
if (dic[505].indexOf(res.toUpperCase()) > -1) {
editopc(o,r-1, nextfunc);
} // voltando...
else {
editopc(o,r, nextfunc);
}
});
} else {
if (!nextfunc) { } else { nextfunc(); }
...
if (!l[r].value) {
var texto = l[r].title;
texto=(l[r].obj.selectedIndex > -1)?dic[445]+': '+texto+' '+dic[462]+': '+l[r].obj.options[l[r].obj.selectedIndex].text+': '+dic[463]:dic[445]+' '+texto+' '+dic[464];
getvoiceconfirm(texto, function() {
editopc(l[r].obj,0, function() { leobj(l,r+1); });
}, function() {
leobj(l,r+1);
}, function() {
fala(dic[442], startcommand);
...
getvoiceconfirm('deseja clicar no item:, '+l[r].text+'?', function() {
l[r].obj.value = l[r].value;
if (l[r].obj.onchange) { l[r].obj.onchange(); }
}, function() {
leobj(l,r+1);
}, function() {
fala(dic[442],startcommand);
...
case 'SUBMIT':
case 'BUTTON':
getvoiceconfirm(px.textContent+': , '+dic[540], function() {
px.click();
}, function() {
leobj(l, r+1);
}, function(res) {
...
break;
case 'TEXTAREA':
var t = dic[445]+': '+l[r].title;
t+=(l[r].obj.value.length < 1)?' '+dic[472]:' '+dic[473]+': '+l[r].obj.value+': '+dic[474];
getvoiceconfirm(t, function() {
getvoice(dic[456]+': ', function(res) {
l[r].obj.value = res;
if (l[r].obj.onchange) { l[r].obj.onchange(); }
fala(dic[457]+': '+res, function() { leobj(l, r+1); });
});
}, function() {
leobj(l, r+1);
}, function() {
...
break;
case 'INPUT':
switch(px.type.toUpperCase()) {
case 'SUBMIT':
case 'BUTTON':
getvoiceconfirm(l[r].title+': , '+dic[540], function() {
px.click();
}, function() {
leobj(l, r+1);
}, function(res) {
...
break;
case 'CHECKBOX':
var texto = dic[445]+' '+l[r].title;
texto+=(l[r].obj.checked == true)?' '+dic[475]:' '+dic[476];
getvoiceconfirm(texto, function() {
if (l[r].obj.checked == true) {
l[r].obj.checked = false;
fala(dic[458], function() { leobj(l,r+1); });
} else {
l[r].obj.checked = true;
fala(dic[459], function() { leobj(l,r+1); });
} // agora marca...
px.onclick();
}, function() {
leobj(l, r+1);
...
} // do segundo case type...
break;
default: leobj(l,r+1);
} // do switch...
} else {
fala(dic[545],startcommand);
} // ultrapassou...
} // da funcao...
...
c = c.toUpperCase();
var l = [];
var p = document.querySelectorAll('a,li,input,button,select,textarea,submit');
if (!p) { 
return l;
} else {
var len = p.length;
...
if (p[i].innerHTML.toUpperCase().indexOf(c) > -1) {
if ((p[i].innerHTML.toUpperCase().indexOf('<A ') < 0) && (p[i].innerHTML.toUpperCase().indexOf('<INPUT ') < 0) && (p[i].innerHTML.toUpperCase().indexOf('<BUTTON ') < 0) && (p[i].innerHTML.toUpperCase().indexOf('<SELECT ') < 0)) {
l.push({ title: p[i].innerHTML+' '+p[i].title, obj: p[i]});
}
} // encontrou e adiciona...
} // tipo direto
else {
if (((p[i].tagName == 'INPUT') && (p[i].type.toUpperCase() == 'BUTTON')) || ((p[i].tagName == 'INPUT') && (p[i].type.toUpperCase() == 'SUBMIT')) || (p[i].tagName == 'BUTTON') || (p[i].tagName == 'SUBMIT')) {
var tx = p[i].value;
if (tx.length < 1) { tx = p[i].innerHTML; }
tx+=' '+p[i].title;
if (tx.toUpperCase().indexOf(c) > -1) {
l.push({ title: tx, obj: p[i]});
} // encontrou...
} // pegando do value...
else {
...
// l.push({ title: d.innerHTML, obj: p[i]});
oo = 1;
} // ja adicionou a lista toda...
} // existe label
if (oo < 1) {
for (var r=0;r<p[i].length;r++) {
if (p[i].options[r].text.toUpperCase().indexOf(c) > -1) {
l.push({ title: '', obj: p[i], text: p[i].options[r].text, value: p[i].options[r].value});
} // encontrou...
} // do for...
} // nao encontrou o titulo, procura nos itens...
} // TIPO select...
} // apenas os visiveis...
} // do for...
} // existe formulario
...
getvoice(texto, function(rr) {
if ((dic[478]).indexOf(rr.toUpperCase()) > -1) {
editopc(l[r],0, function() { leform(l, r+1); });
} else {
leform(l, r+1); 
} // nao altera...
});
