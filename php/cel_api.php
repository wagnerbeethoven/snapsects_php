...
if (($act == "gettext") && (isset($_POST["id"]))) {
    ...
    if (strlen($ee->photo) > 0) {
        $t .= '<br><img id="grpi" src="' . $site_url . $ee->photo . '" alt="{dic219} ' . $ee->commonname . ' (' . $ee->scientificname . ') - ' . $ee->photodesc . '" width="500px" onload="redimensiona();" /><br>';
    }
    $t .= '<br><h2>{dic220}</h2><br>';
    $t .= '{dic221}: ' . $ee->author . '<br>';
    if (strlen($ee->email) > 0) {
        $t .= '{dic187}: <a href="mailto:' . $ee->email . '">' . $ee->email . '</a><br>';
    }
    if (strlen($ee->url) > 0) {
        $t .= '{dic189}: <a href="' . $ee->url . '">' . $ee->url . '</a><br>';
    }
    if (strlen($ee->colaborative) > 0) {
        $t .= '{dic191}: ' . $ee->colaborative . '<br>';
    }
    $t .= '<br><h2>{dic222}</h2><br>';
...
if ($act == "sendemail") {
    ...
    // $logo = '<img src="'.$site_url.'img/logo_application.jpeg" width="150px"><hr>';
    // $img = $_POST['photo'];
    // if (strlen($img) > 0) {
    // $nnomme = $email;
    // $diretorio = 'temp/';
    ...
    // $texto = $logo.'<img src=$site_url.$file.'" width="300px"><br>'.$texto;
    // }
    ...
    // $texto = '<img src="'.$site_url.'img/logo_application.jpeg" width="300px">';
    // $texto .= 'Thanks for your participation!!!<br>';
