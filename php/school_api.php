...
require_once "scripts/snapsect_userclass.php";
...
echo JSON_encode(array('salas' => getsalas($email)),JSON_UNESCAPED_UNICODE);
} // do get sala...
...
echo JSON_encode(array('exerciceos' => $l, 'leituras' => $ll, 'alunos' => $lll, 'provas' => $provas),JSON_UNESCAPED_UNICODE);
$e = 1;
...
if ($e == 0) {
$px = array();
echo JSON_encode(array('exerciceos' => $px, 'leituras' => $px),JSON_UNESCAPED_UNICODE);
} // nao envia nada...
} // do getshow...
...
echo JSON_encode(array('status' => 'ok', 'results' => $id), JSON_UNESCAPED_UNICODE);
setindex($email, $sala);
// incluir aqui o index...
} else {
echo JSON_encode(array('status' => 'error','results' => 'no email'), JSON_UNESCAPED_UNICODE);
...
echo JSON_encode(array('status' => $status, 'licao' => $licao),JSON_UNESCAPED_UNICODE);
} // da funcao getquest
...
echo JSON_encode(array('status' => 'ok', 'licao' => $licao),JSON_UNESCAPED_UNICODE);
} // arquivo existe...
} // email existe
...
echo JSON_encode(array('status' => 'error', 'errorcode' => 'no email'),JSON_UNESCAPED_UNICODE);
} // nao tem sala...
} else {
echo JSON_encode(array('status' => 'error', 'errorcode' => 'no email'),JSON_UNESCAPED_UNICODE);
} // nao tem aluno
...
if (count($ll) > 0) {
echo JSON_encode(array('status' => 'ok', 'results' => $ll),JSON_UNESCAPED_UNICODE);
} else {
echo JSON_encode(array('status' => 'none'),JSON_UNESCAPED_UNICODE);
}
} else {
echo JSON_encode(array('status' => 'none'),JSON_UNESCAPED_UNICODE);
} // nao tem convites...
...
echo JSON_encode(array('status' => 'ok', 'results' => $ll, 'leituras' => $leituras),JSON_UNESCAPED_UNICODE);
} // existe sala...
...
echo JSON_encode(array('status' => 'ok', 'leitura' => $l,JSON_UNESCAPED_UNICODE));
} // existe arquivo...
} // do id
} // da sala
} // do email...
...
echo JSON_encode(array('status' => 'error','results' => 'no sala directory'),JSON_UNESCAPED_UNICODE);
} // diretorio da sala nao existe.
} else {
echo JSON_encode(array('status' => 'error','results' => 'no sala'),JSON_UNESCAPED_UNICODE);
} // sala nao existe.
} else {
echo JSON_encode(array('status' => 'error','results' => 'no directory '.$email),JSON_UNESCAPED_UNICODE);
} // nao existe o diretorio
} else {
echo JSON_encode(array('status' => 'error','results' => 'no email')),JSON_UNESCAPED_UNICODE;
} // email vazio...
...
echo JSON_encode(array('status' => $re, 'results' => $resultado),JSON_UNESCAPED_UNICODE);
} // da funcao...
...
echo JSON_encode(array('status' => $status, 'licao' => $llicao),JSON_UNESCAPED_UNICODE);
} // da funcao getquest