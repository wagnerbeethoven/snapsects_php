...
if ($_SESSION["ssid"] > 0) {
    $tpl = new Template('templates/layout.html', false, 'templates/partial/add_snapsect.html', 'SnapSect - Administrator');
    if (isset($_GET["id"]))
    ...
    if (($_SESSION["ssid"] == $ee->userid) || ($_SESSION["ssstatus"] == 67)) {
        $tpl->block("BLOCK_TAB");
        $tpl->block("BLOCK_MENU_ARTROPOD_ADM_JS");
        $tpl->block('BLOCK_ADM');
    } else {
            $tpl->block("BLOCK_MENU_ARTROPOD_JS");
