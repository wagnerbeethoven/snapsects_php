...
t+='" onclick="javascript:showimg('+r+');">';
} // do for...
} // da funcao...
} // existe photos
if (!ee.videos) { } else {
...
t+='">';
t+='  <input type="button" onclick="javascript:myconfirm('+"'SnapSects School',dic[339],['"+dic[249]+"','"+dic[248]+"'], function(res) { if (res == '1') {delvideo("+r+');} });" value="Remover vídeo">';
} // do for...
} // da funcao...
} // existe photos
...
t+='<p>'+dic[270]+'</p>';
t+='<br><center><h2 onclick="'+"javascript:document.getElementById('dvs0').style.display = (document.getElementById('dvs0').style.display == 'none')? 'inline': 'none';"+'">'+dic[225]+'</h2></center><br>';
t+='<div id="dvs0" style="display:none">';
t+=dic[153]+': '+ee.collecting+'<br>';
t+=dic[155]+': '+ee.place+'<br>';
t+=dic[157]+': '+ee.win+'<br>';
t+=dic[159]+': '+ee.address+'<br>';
t+=dic[160]+': '+ee.lat+'<br>'; 
t+=dic[161]+': '+ee.lng+'<br>';
t+=dic[162]+': '+ee.city+'<br>';
t+=dic[163]+': '+ee.uf+'<br>';
t+=dic[164]+': '+ee.country+'<br>';
t+=dic[165]+': '+ee.datacad+'<br>';
t+=dic[166]+': '+ee.temperature+'<br>';

t+='</div>';
if (ee.notes.length > 0) {
t+='<p>'+dic[270]+'</p>';
t+='<br><center><h2 onclick="'+"javascript:document.getElementById('dvs1').style.display = (document.getElementById('dvs1').style.display == 'none')? 'inline': 'none';"+'">'+dic[170]+'</h2></center><br>';
t+='<div id="dvs1" style="display:none">';
t+='<p align="justify">'+ee.notes.replaceAll('\\n','<br>')+'</p><br>';
t+='</div>';
}
if (ee.audiodesc.length > 0) {
t+='<p>'+dic[270]+'</p>';
t+='<br><center><h2 onclick="'+"javascript:document.getElementById('dvs2').style.display = (document.getElementById('dvs2').style.display == 'none')? 'inline': 'none';"+'">'+dic[174]+'</h2></center><br>';
t+='<div id="dvs2" style="display:none">';
t+='<p align="justify">'+ee.audiodesc.replaceAll('\\n','<br>')+'</p><br>';
t+='</div>';
}
if (ee.mediation.length > 0) {
t+='<p>'+dic[270]+'</p>';
t+='<br><center><h2 onclick="'+"javascript:document.getElementById('dvs3').style.display = (document.getElementById('dvs3').style.display == 'none')? 'inline': 'none';"+'">'+dic[168]+'</h2></center><br>';
t+='<div id="dvs3" style="display:none">';
t+='<p align="justify">'+ee.mediation.replaceAll('\\n','<br>')+'</p><br>';
t+='</div>';
} // 
if (ee.water.length > 0) {
t+='<p>'+dic[270]+'</p>';
t+='<br><center><h2 onclick="'+"javascript:document.getElementById('dvs4').style.display = (document.getElementById('dvs4').style.display == 'none')? 'inline': 'none';"+'">'+dic[177]+'</h2></center><br>';
t+='<div id="dvs4" style="display:none">';
...
snd.stopRecord();
if (res == 1) {
navigator.notification.prompt('Digite o título do áudio:', function(r) {
